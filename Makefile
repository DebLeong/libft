# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: dleong <marvin@42.fr>                      +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2017/09/19 22:50:05 by dleong            #+#    #+#              #
#*   Updated: 2017/09/19 23:43:41 by dleong           ###   ########.fr       *#
#                                                                              #
# **************************************************************************** #

NAME = libft.a

FILES = ft_lstdelone.c ft_putnbr_fd.c ft_strmapi.c ft_lstiter.c ft_putstr.c \
	ft_strncat.c ft_atoi.c ft_lstmap.c ft_putstr_fd.c ft_strncmp.c \
	ft_bzero.c ft_lstnew.c ft_strcat.c ft_strncpy.c ft_gc.c ft_memalloc.c \
	ft_strchr.c ft_strnequ.c ft_getdigit.c ft_memccpy.c ft_strclr.c ft_strnew.c\
	ft_gl.c ft_memchr.c ft_strcmp.c ft_strnstr.c ft_isalnum.c ft_memcmp.c \
	ft_strcpy.c ft_strrchr.c ft_isalpha.c ft_memcpy.c ft_strdel.c ft_strsplit.c\
	ft_isascii.c ft_memdel.c ft_strdup.c ft_strstr.c ft_isdigit.c ft_memmove.c \
	ft_strequ.c ft_strsub.c ft_isprint.c ft_memset.c ft_striter.c ft_strtrim.c \
	ft_istrimspace.c ft_putchar.c ft_striteri.c ft_tolower.c ft_iswhitespace.c \
	ft_putchar_fd.c ft_strjoin.c ft_toupper.c ft_itoa.c ft_putendl.c \
	ft_strlcat.c ft_lstadd.c ft_putendl_fd.c ft_strlen.c ft_lstdel.c \
	ft_putnbr.c	ft_strmap.c

OBJ = *.o

all: $(NAME)

$(OBJ): $(FILES)
	gcc -Wall -Wextra -Werror -c $(FILES)

$(NAME): $(OBJ)
	ar rcs $(NAME) $(OBJ)

clean:
	rm -f $(OBJ)

fclean: clean
	rm -f $(NAME)

re: fclean all

.PHONY: clean fclean re all
